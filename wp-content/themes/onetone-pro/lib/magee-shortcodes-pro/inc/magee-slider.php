...
// slider
 ...
 function magee_slider_register() {
	...
	$labels = array(
		'name' => __('Magee Slider','onetone'),
		'singular_name' => 'Magee Slider',
		'add_new_item' => __('Add New Slider','onetone'),
		'edit_item' => __('Edit Slider', 'onetone'),
		'new_item' => __('New Slider', 'onetone'),
		'view_item' => '',
		'all_items' => 'All Sliders',
		'search_items' => __('Search Slider', 'onetone'),
	...
	foreach( $slider as $slide ):
		...
		<li id="listItem_<?php echo $i ?>"  class="ui-state-default" style="margin-bottom:10px;">
			<div class="widget-content option-item" style="margin:5px; padding:5px;">
				...
				<label for="magee_custom_slider[<?php echo $i ?>][title]"><span><?php _e("Slide Title",'onetone');?> :</span><input id="magee_custom_slider[<?php echo $i ?>][title]" name="magee_custom_slider[<?php echo $i ?>][title]" value="<?php  echo stripslashes( $slide['title'] )  ?>" type="text" /></label>
				
				<label for="magee_custom_slider[<?php echo $i ?>][caption]"><span style="float:left" ><?php _e("Slide Caption",'onetone');?> :</span><textarea name="magee_custom_slider[<?php echo $i ?>][caption]" id="magee_custom_slider[<?php echo $i ?>][caption]"><?php echo stripslashes($slide['caption']) ; ?></textarea></label>
				<input id="magee_custom_slider[<?php echo $i ?>][id]" name="magee_custom_slider[<?php echo $i ?>][id]" value="<?php  echo $slide['id']  ?>" type="hidden" />
                <a class="sort-item"><?php _e("Sort",'onetone');?></a>
				<a class="del-item"><?php _e("Delete",'onetone');?></a>
				<div class="clear"></div>
			</div>
		</li>
	<?php endforeach; 
	}else{
		echo ' <p> '.__("Use the button above to add Slides !",'onetone').'</p>';
	}?>
	</ul>
	<div class="clear"></div>
  ...
  <?php
	...
	function magee_slider_edit_columns($columns){
	  $columns = array(
		...
		"slides" => __("Number of Slides",'onetone'),
