		...
		if ( $force_refresh || 
			...
			if ( $response['response']['code'] == 200 ) {
				...
				if ( $data->status === 'OK' ) {
				  	...
				  	return __( 'No location found for the entered address.', 'onetone');
				} elseif( $data->status === 'INVALID_REQUEST' ) {
				   	return __( 'Invalid request. Did you enter an address?', 'onetone');
				} else {
					return __( 'Something went wrong while retrieving your map, please ensure you have entered the short code correctly.', 'onetone');
				}

			} else {
			 	return __( 'Unable to contact Google API service.', 'onetone');
			}
